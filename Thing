        ...
        if v ~= plr then
            pcall(function()
                ...
                if (ms.Hit.Position - v.Character.PrimaryPart.Position).magnitude < distance and checkPassed then
                    if canSee(v.Character.Torso, v.Character) and v.Character.Humanoid.Health > 0 then
                        closest = v
...
while task.wait() do
    if aimbotOn then
        ...
        if targ then
            ...
            workspace.CurrentCamera.CFrame = currentcf:Lerp(CFrame.new(currentcf.Position, targ.Character.Torso.Position), _G.AimbotEasing)
        end
    end
end
